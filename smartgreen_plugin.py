...
class SmartGreenPlugin:
	...
	def __init__(self, iface):
		...
		if self.settings['qgis.dblite'][0]!='':
			...
			self.DBM = SQLiteDriver(self.settings['qgis.dblite'][0],progress = self.smDock)
	...
	def initDockPanel(self):
		if self.smDock is None: self.smDock = SMDock(self.iface,self,None)
		self.showDockPanel()
	...
	def moveToOthers(self,layList):
		...
		#layList = QgsMapLayerRegistry.instance().mapLayers().values()
		...
		for lay in layList:
			if not self.SGsettings.valueExist(lay.source()):
				...
				QgsMapLayerRegistry.instance().addMapLayer(clonedLay, False)
	...
	def removeMapLayers(self, layerIds):
		for ids in layerIds:
			lyr = QgsMapLayerRegistry.instance().mapLayer(ids)  # returns QgsMapLayer pointer
			if lyr is not None:
	...
	def setNewSimulation(self):
		...
		# create and show the dialog 
		dlg = SimulationInfoDialog(self.tr) 
		# show the dialog
		dlg.show()
		result = dlg.exec_() 
		# See if OK was pressed
	...
	def addEmptyLayers(self, filename):
		...
		prjCrs = canvas.mapRenderer().destinationCrs()
	...
	def saveNetworkSF(self, name):
		...
		self.DBM.addVectorTable(name, fieldList, typeList, QGis.WKBMultiLineString)
	...
	def saveNodesSF(self, name):		
		...
		self.DBM.addVectorTable(name, fieldList, typeList, QGis.WKBMultiPoint)
		return self.DBM.getTableSource(name)
	...
	def saveSoilslayerSF(self, name):
		...
		self.DBM.addVectorTable(name, fieldList, typeList, QGis.WKBMultiPolygon)
		return self.DBM.getTableSource(name)
	...
	def importFromVector(self, toLayer = None, maskLayer= None):
		...
		if self.compareLayerSource(toLayer.source(),self.DBM.getDefault('qgis.networklayer')):
			...
			dlg = ImportDialog(title = self.tr('Import into %s from existing layer'%(toLayer.name())),
										layType='polygon',settings = self.DBM,
										importTableName = os.path.join(self.plugin_dir,'tables','importlidlayer.csv'),
										tr = self.tr)
		else:
		...
		if result == 1: 
			...
			if maskLayer is not None:
				selId = selectByLocation(fromLayer,maskLayer,0,['intersects'],precision=0.001, progress = None)
				fromLayer.setSelectedFeatures(selId)
			...
			# add feature
			newFeat = QgsFeature(toLayer.pendingFields())

			for feat in fromLayer.getFeatures():
				newFeat.setGeometry(feat.geometry())
				flag = toLayer.addFeatures([newFeat],True)
	...
	def runAsThread(self, function):
		...
		s = QSettings()
		if s.value('SMARTGREEN/debugMode', 'False')=='True':
		...
		try:
			...
			self.thread = QThread()
			self.worker = Worker(function, progress= None)
	...
	def exportGISData(self,progress= None):
		...
		centerPt = ext.center()
		sourceCrs = self.iface.mapCanvas().mapRenderer().destinationCrs()
		destCrs = QgsCoordinateReferenceSystem(4326)
		tr = QgsCoordinateTransform(sourceCrs, destCrs)
		centerPt = tr.transform(centerPt)
		dataToMat(filename = self.DBM.getDefault('gisdatapath'),data = centerPt.x(),name= 'blon',progress = progress)
		dataToMat(filename = self.DBM.getDefault('gisdatapath'),data = centerPt.y(),name= 'blat',progress = progress)
		...
		dataToMat(filename = self.DBM.getDefault('gisdatapath'),data = mask.count(),name= 'A',progress = progress)
		...
		fieldIdx =  inputLayer.fieldNameIndex(self.DBM.getDefault('qgis.nodeslayer.field.elev_top'))
		...
		#zr.saveAsASC(filename = osp.join(tempPath,'zr.asc'),progress = progress)		
		# save max value of zr
		zrmax = zr.max()
		dataToMat(filename = self.DBM.getDefault('gisdatapath'),data = zrmax,name= 'zrmax',progress = progress)
		# save the index corresponding to zrmax
		ifoc = zr.getIndex(zrmax)
		dataToMat(filename = self.DBM.getDefault('gisdatapath'),data = ifoc,name= 'ifoc',progress = progress)
		# save index of flow path
		k1,k2,k3,k4,k5,k6,k7,k8,st8 = HYD.stack8point(flowDirMobi)
		
		dataToMat(filename = self.DBM.getDefault('gisdatapath'),data = k1,name= 'sk1',progress = progress)
		dataToMat(filename = self.DBM.getDefault('gisdatapath'),data = k2,name= 'sk2',progress = progress)
		dataToMat(filename = self.DBM.getDefault('gisdatapath'),data = k3,name= 'sk3',progress = progress)
		dataToMat(filename = self.DBM.getDefault('gisdatapath'),data = k4,name= 'sk4',progress = progress)
		dataToMat(filename = self.DBM.getDefault('gisdatapath'),data = k5,name= 'sk5',progress = progress)
		dataToMat(filename = self.DBM.getDefault('gisdatapath'),data = k6,name= 'sk6',progress = progress)
		dataToMat(filename = self.DBM.getDefault('gisdatapath'),data = k7,name= 'sk7',progress = progress)
		dataToMat(filename = self.DBM.getDefault('gisdatapath'),data = k8,name= 'sk8',progress = progress)
		dataToMat(filename = self.DBM.getDefault('gisdatapath'),data = st8,name= 'st8',progress = progress)
		
		# channelized flow fraction
		threchan=0 
		dataToMat(filename = self.DBM.getDefault('gisdatapath'),data = threchan,name= 'threchan',progress = progress)
		...
		aann=1
		dataToMat(filename = self.DBM.getDefault('gisdatapath'),data = aann,name= 'aann',progress = progress)
		...
		#Wg0.saveAsASC(filename = osp.join(tempPath,'Wg0beforelids.asc'),progress = progress)
		# overwrite where lids are defined
		self.rasterizeParam(Wg0,self.getLayerBySource(self.DBM.getDefault('qgis.lidlayer')),'',1,progress,LE.Wg0)
		#Wg0.saveAsASC(filename = osp.join(tempPath,'Wg0afterlids.asc'),progress = progress)
		Wg0.saveAsMAT(filename = self.DBM.getDefault('gisdatapath'),name= 'Wg0',progress = progress)
		#Wg0 = Wg0*1000
		#Wg0.saveAsASC(filename = osp.join(tempPath,'Wg0.asc'),progress = progress)
		...
		#Wc0.saveAsASC(filename = osp.join(tempPath,'Wc0beforelids.asc'),progress = progress)
		# overwrite where lids are defined
		self.rasterizeParam(Wc0,self.getLayerBySource(self.DBM.getDefault('qgis.lidlayer')),'',1,progress,LE.Wc0)
		#Wc0.saveAsASC(filename = osp.join(tempPath,'Wc0afterlids.asc'),progress = progress)
		...
		#ks.saveAsASC(filename = osp.join(tempPath,'ksafterlids.asc'),progress = progress)
		...
		#Wp0.saveAsASC(filename = osp.join(tempPath,'Wp0beforelids.asc'),progress = progress)
		# overwrite where lids are defined
		self.rasterizeParam(Wp0,self.getLayerBySource(self.DBM.getDefault('qgis.lidlayer')),'',1,progress,LE.Wp0)
		#Wp0.saveAsASC(filename = osp.join(tempPath,'Wp0afterlids.asc'),progress = progress)
		
		Wp0.saveAsMAT(filename = self.DBM.getDefault('gisdatapath'),name= 'Wp0',progress = progress)
		#Wp0.saveAsASC(filename = osp.join(tempPath,'Wp0.asc'),progress = progress)
		...
		#Alb.saveAsASC(filename = osp.join(tempPath,'Alb.asc'),progress = progress)
		...
		#kf.saveAsASC(filename = osp.join(tempPath,'kf.asc'),progress = progress)
		...
		#Ma.saveAsASC(filename = osp.join(tempPath,'Ma.asc'),progress = progress)
		...
		#Mf.saveAsASC(filename = osp.join(tempPath,'Mf.asc'),progress = progress)
		
		# set initial conditions
		Ws0  = mask.copy(0.0)
		Ws0.saveAsMAT(filename = self.DBM.getDefault('gisdatapath'),name= 'Ws0',progress = progress)
		#Ws0.saveAsASC(filename = osp.join(tempPath,'Ws0.asc'),progress = progress)
		...
		#cha_subflow.saveAsASC(filename = osp.join(tempPath,'cha_subflow.asc'),progress = progress)
		...
		for i,lab in enumerate(labList):
			dataToMat(filename = self.DBM.getDefault('gisdatapath'),data = emptyArr,name= lab,progress = progress)
	...
	def createSubCatchments(self):
		...
		outLink = netLayer.getFeatures(QgsFeatureRequest().setFilterFid(outLinkId[0])).next()
		...
		for feat in scLayer.getFeatures():
			scLayer.changeAttributeValue(feat.id(), scLayer.fieldNameIndex(self.DBM.getDefault('qgis.subcatchmentslayer.field.obj_id')), '%s%s'%('S',feat.id()))
			scLayer.changeAttributeValue(feat.id(), scLayer.fieldNameIndex(self.DBM.getDefault('qgis.subcatchmentslayer.field.wg0')), self.DBM.getDefault('param_default.Wg0'))
			scLayer.changeAttributeValue(feat.id(), scLayer.fieldNameIndex(self.DBM.getDefault('qgis.subcatchmentslayer.field.wc0')), self.DBM.getDefault('param_default.Wc0'))
			scLayer.changeAttributeValue(feat.id(), scLayer.fieldNameIndex(self.DBM.getDefault('qgis.subcatchmentslayer.field.wp0')), self.DBM.getDefault('param_default.Wp0'))
			scLayer.changeAttributeValue(feat.id(), scLayer.fieldNameIndex(self.DBM.getDefault('qgis.subcatchmentslayer.field.ks')), self.DBM.getDefault('param_default.ks'))
			scLayer.changeAttributeValue(feat.id(), scLayer.fieldNameIndex(self.DBM.getDefault('qgis.subcatchmentslayer.field.ch')), self.DBM.getDefault('param_default.CH'))
			scLayer.changeAttributeValue(feat.id(), scLayer.fieldNameIndex(self.DBM.getDefault('qgis.subcatchmentslayer.field.alb')), self.DBM.getDefault('param_default.Alb'))
			scLayer.changeAttributeValue(feat.id(), scLayer.fieldNameIndex(self.DBM.getDefault('qgis.subcatchmentslayer.field.kf')), self.DBM.getDefault('param_default.kf'))
			scLayer.changeAttributeValue(feat.id(), scLayer.fieldNameIndex(self.DBM.getDefault('qgis.subcatchmentslayer.field.ma')), 0)
			scLayer.changeAttributeValue(feat.id(), scLayer.fieldNameIndex(self.DBM.getDefault('qgis.subcatchmentslayer.field.mf')), 0)
		...
		if result == 1: 
			...
			for tableName in tableNames:
				...
				if actionName == actionList[0]:
					...
					if reply == QMessageBox.Yes:
						...
						if tableLay is not None:
							QgsMapLayerRegistry.instance().removeMapLayers( [tableLay.id()] )
				...
				if actionName == actionList[1]:
					...
					#Add the layer to the QGIS Map Layer Registry
					QgsMapLayerRegistry.instance().addMapLayer(tableLay, False)
	...
	def addRemoveDummyFeature(self, lay):
		lay.startEditing()
		# add feature
		newFeat = QgsFeature(lay.pendingFields())
	...
	def setFormUI(self,mylayer,formUi):
		if formUi is not None:
			...
			mylayer.setEditorLayout( QgsVectorLayer.UiFileLayout )
			mylayer.setEditFormInit(os.path.join(self.plugin_dir,'layerforms',formUi+'.py'))
			mylayer.editFormConfig().setInitCodeSource(1)
			mylayer.editFormConfig().setInitFilePath(os.path.join(self.plugin_dir,'layerforms',formUi+'.py'))
			mylayer.setEditForm(os.path.join(self.plugin_dir,'layerforms',formUi+'.ui'))
			mylayer.setEditFormInit('formOpen')
	...
	def altimetricChart(self):
		...
		if len(x)>0:
			...
			self.showCriticalMessageBox(text = self.tr('Cannot show elevation profile'),infoText=self.tr('Not enought selected links!'),detailText=self.tr('Please select at least one link.'))
	...
	def findDuplicates(self,progress= None):
		...
		checker = CheckData(self.getLayerBySource(self.DBM.getDefault('qgis.networklayer')),self.getLayerBySource(self.DBM.getDefault('qgis.nodeslayer')),progress)
	...
	def fixLinksElevations(self,progress = None):
		from tools.check_data import CheckData
			...
			if rLayer.isValid():
				...
				for selectedFeat in vLayer.selectedFeatures():
					...
					if len(res)>0:
						...
						# save val to vLayer
						fldIdx = selectedFeat.fields().fieldNameIndex(fieldList[i])
	...
	def maxNetworkTravelTime(self):
		...
		linkLay.setSelectedFeatures(maxTravelIds)
		
		msg = QMessageBox()
		msg.setIcon(QMessageBox.Information)
		msg.setText('Estimated travel time is %s (in minutes)'%int(maxTravelTime/60))
		msg.setInformativeText('Selection shows the maximum path')
		msg.setWindowTitle('SMARTGREEN')
		#msg.setDetailedText(detailText)
		msg.setStandardButtons(QMessageBox.Ok)
		msg.exec_()
	...
	def calculateTravelTime(self,netLayer,diamFld,mannFld,startElevFld,endElevFld,lengthFld):
		...
		for feat in netLayer.selectedFeatures():
			...
			radius = diam/4.0
			# calculate slope
			slope = (startElev-endElev)/length
			if slope <=0:
				self.smDock.setInfo('<a href="find;%s;%s">Link %s</a> has negative slope %s'%(netLayer.id(),feat.id(),feat['OBJ_ID'],slope), error = True)
				slope = 0.001
			...
			if flowVel>0:
				...
				self.smDock.setInfo('<a href="find;%s;%s">Link %s</a> has negative flow velocity %s'%(netLayer.id(),feat.id(),feat['OBJ_ID'],flowVel), error = True)
	...
	def makeStatistics(self):
		...
		for link in links.getFeatures():
			if isinstance(link['ELEV_START'],QPyNullVariant): elevStart.append(np.nan)
			else: elevStart.append(link['ELEV_START'])
			if isinstance(link['ELEV_END'],QPyNullVariant): elevEnd.append(np.nan)
			else: elevEnd.append(link['ELEV_END'])
			if isinstance(link['LENGTH'],QPyNullVariant): length.append(np.nan)
	...
	def showCalibrationDataDialog(self):
		from forms.calibration_data_mainwindow import CalibrationDataMainwindow
		self.dlg = CalibrationDataMainwindow(self.plugin_dir,self.DBM,None,None)
		self.dlg.show()
