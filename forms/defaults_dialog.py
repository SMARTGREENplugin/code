...
class DefaultsDialog(QtGui.QDialog):
	...
	def setupTab(self,tabName,paramList):
		...
		tab.setWidget(QtGui.QWidget())
		tab_layout = QtGui.QFormLayout(tab.widget())
		tab.setWidgetResizable(True)

		for param in paramList:
			labValue = QtGui.QLabel(param[1])
			labValue.setWhatsThis(param[2])
			textValue = QtGui.QLineEdit()
	...
	def updateFromList(self,paramList):
		for param in paramList:
			paramFld = self.findChild(QtGui.QLineEdit,param[0])
			paramValue = unicode(paramFld.text())
			# update value
