...
class StringInput(QtGui.QWidget):
...
class NumericInput(QtGui.QWidget):
...
class FieldInput(QtGui.QWidget):
	def __init__(self, objName, labelString, defaultValue,descr = '',unit = None):
		...
		self.setWhatsThis(descr)
		self.label = QtGui.QLabel(labelString)
		self.comboField = QtGui.QComboBox()
		self.comboUnit = QtGui.QComboBox()
...
class MultiListInput(QtGui.QWidget):
...
class FileInput(QtGui.QWidget):
	def __init__(self, labelString, defaultValue, type,descr = ''):
		...
		# link action
		if (type == 'openraster'): QtCore.QObject.connect(self.button, QtCore.SIGNAL('clicked()'), self.openRaster)
		if (type == 'saveraster'): QtCore.QObject.connect(self.button, QtCore.SIGNAL('clicked()'), self.saveRaster)
		if (type == 'openvector'): QtCore.QObject.connect(self.button, QtCore.SIGNAL('clicked()'), self.openVector)
		if (type == 'opensqlite'): QtCore.QObject.connect(self.button, QtCore.SIGNAL('clicked()'), self.openSqlite)
		if (type == 'opencsv'): QtCore.QObject.connect(self.button, QtCore.SIGNAL('clicked()'), self.openCSV)
	...
	def openSqlite(self):
		...
		inputFile = QtGui.QFileDialog.getOpenFileName(None, self.label.text(), '', 'sqlite(*.sqlite)')
		if not inputFile:
			return
		# update input box
		self.input.setText(inputFile)
	...
	def openCSV(self):
		#inputFile = Utils.FileDialog.getOpenFileName( self, self.label.text(), 'sqlite(*.sqlite)', 'sqlite(*.sqlite)' )
		inputFile = QtGui.QFileDialog.getOpenFileName(None, self.label.text(), '', 'Comma Separated Values (*.CSV)')
		if not inputFile:
			return
		# update input box
		self.input.setText(inputFile)

class LayerInput(QtGui.QWidget):
...
class VectorLayerInput(QtGui.QWidget):
	valueChanged = QtCore.pyqtSignal('QString')
	
	def __init__(self,labelString, descr = '', type = 'any', showOpenFile = True):
		super(VectorLayerInput, self).__init__()
		if type in ['any','all']:
			self.type = [QGis.WKBUnknown, QGis.WKBPoint, QGis.WKBLineString, QGis.WKBPolygon, \
							QGis.WKBMultiPoint, QGis.WKBMultiLineString, QGis.WKBMultiPolygon, QGis.WKBNoGeometry, \
							QGis.WKBPoint25D, QGis.WKBLineString25D, QGis.WKBPolygon25D, QGis.WKBMultiPoint25D, \
							QGis.WKBMultiLineString25D, QGis.WKBMultiPolygon25D ]
		elif type == 'point':
			self.type = [QGis.WKBPoint, QGis.WKBMultiPoint,QGis.WKBPoint25D,QGis.WKBMultiPoint25D]
		elif type == 'line':
			self.type = [QGis.WKBLineString, QGis.WKBMultiLineString, QGis.WKBLineString25D]
		elif type == 'polygon':
			self.type = [QGis.WKBPolygon, QGis.WKBMultiPolygon, QGis.WKBPolygon25D, QGis.WKBMultiPolygon25D ]
		else:
		...
		if showOpenFile:
			...
			self.button.setText('...')
			grid.addWidget(self.button, 2, 1)
			QtCore.QObject.connect(self.button, QtCore.SIGNAL('clicked()'), self.openVector)
