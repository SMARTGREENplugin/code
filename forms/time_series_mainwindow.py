...
class TimeSeriesMainwindow(QMainWindow):
	...
	def addNewTable(self):
		...
		if self.weatherstationslayer:
			...
			if result == 1: 
				...
				tableIdx = self.weatherstationslayer.fieldNameIndex('TABLE')
				...
				for feature in selection:
					...
					hyetograph = createHyetograph(duration = duration, step = step, \
																par_a1 = feature['A1'], par_n = feature['N'], par_alp = feature['ALP'], par_eps = feature['EPS'], par_kap = feature['KAP'],\
																par_Tr = returnTime,\
	...
	def importNewTable(self):
		...
		dlg = ImportCsvDialog(self.DBM, self.tr) 
		# show the dialog
		dlg.show()
		result = dlg.exec_() 
		# See if OK was pressed
		if result == 1: 
			res = dlg.getParameterValue()
			filename = res[0]
			newTablename = basename(filename)
			colSep = res[1]
	...
	def updateDataTable(self,item):
		if item:
			varName = str(item.text())
			self.dataArray = ArrayTableModel(parent=None, data = [], header = [self.tr('time (min)'), self.tr('intensity (mm/h)'),self.tr('heigth (mm)')])
	...
	def updateChart(self):
		...
		if self.dataArray:
			...
			self.mw.chartTab.setAxes(xlabs = None, ylabs = None, xTitle = self.tr('time (min)'), yTitle = self.tr('intensity (mm/h)'), y2Title = self.tr('cumulative (mm)'), mainTitle = None)
