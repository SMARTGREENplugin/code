...
class GisGrid():
	...
	#				s = s + '\n'
			...
			if progress is not None: progress.setInfo('Grid exported to %s'%(filename))
		except IOError:
			#print 'Cannot save file: %s' %filename
			if progress is not None: progress.error('Cannot save to %s because $s' %(filename,str(IOError)))
		finally:
			f.close()
	...
	def saveAsMAT(self,filename,name,progress = None):
		...
		try:
			...
			if progress is not None: progress.setInfo('Grid %s exported to %s'%(name,filename),False)
		except IOError:
			#print 'Cannot save file: %s' %filename
	...
	def openASC(self,filename):
		...
		try:
			...
			if (len(tempdata) == (self.nrows*self.ncols)):
				...
				print 'File %s data are not completed' %filename
				# clear all
				self.data = []
		except IOError:
