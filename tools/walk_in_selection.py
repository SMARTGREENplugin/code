...
class walkInSelection:
	...
	def getData(self):
		...
		for id in ids:
			...
			d = feature['DIAM']
			if isinstance(d, QPyNullVariant):
				d = feature['DIM1']
			diams.append(d)
			...
			if isFirst:
				...
				if node:
					...
					surf = node['AREA']
					if isinstance(surf, QPyNullVariant):
						dim = 1.0
					else:
...
class flowTrace:
    ...
    def select(self,xpos=None,ypos=None):
        ...
        # get layer type
        if self.lay.wkbType()!=QGis.WKBLineString and self.lay.wkbType()!=QGis.WKBMultiLineString and self.lay.wkbType()!=QGis.WKBLineString25D and self.lay.wkbType()!=QGis.WKBMultiLineString25D:
          return None
        ...
        while selection_list:
            ...
            feature = self.lay.getFeatures(request).next()            
