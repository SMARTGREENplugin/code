...
class DrainageGeomExporter():
	...
	def featToCoordArray(self,feature):
		...
		if geomType==QGis.WKBMultiPoint:
		...
		if geomType==QGis.WKBMultiLineString:
		...
		if geomType==QGis.WKBMultiPolygon:
			if self.progress is not None: progress.setInfo('Unmanaged layer type: %s (multi-polygon)'%(geomType),True)
			return

		if geomType==100:
			if self.progress is not None: progress.setInfo('Unmanaged layer type: %s (data-only)'%(geomType),True)
			return
	...
	def sectionGeometry(self, sec_type,diam,d1,d2,d3,d4,mann,slope):
		# set NULL to np.nan
		if isinstance(diam, QPyNullVariant): diam = np.nan
		if isinstance(d1, QPyNullVariant): d1 = np.nan
		if isinstance(d2, QPyNullVariant): d2 = np.nan
		if isinstance(d3, QPyNullVariant): d3 = np.nan
		if isinstance(d4, QPyNullVariant): d4 = np.nan
		if isinstance(mann, QPyNullVariant): mann = np.nan
		if isinstance(slope, QPyNullVariant): slope = np.nan
	...
	def linksToMat(self,matFileName):
		...
		for feat in features:
			...
			slope = np.array([[(feat[self.f_z1]-feat[self.f_z2])/feat[self.f_length]]])
			...
			geometry = self.sectionGeometry(feat[self.f_shape],feat[self.f_diam],feat[self.f_d1],feat[self.f_d2],feat[self.f_d3],feat[self.f_d4],feat[self.f_mann],(feat[self.f_z1]-feat[self.f_z2])/feat[self.f_length])
			...
			n1 = np.array([[node.id()]])
			offset1 = np.array(feat[self.f_z1]-node[self.f_zb])
			
			node = self.getNodeByID(feat[self.f_n2])
			#n2 = np.array([[node.id()]], dtype=np.uint16)
			n2 = np.array([[node.id()]])
			offset2 = np.array(feat[self.f_z2]-node[self.f_zb])
