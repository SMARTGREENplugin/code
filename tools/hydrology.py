...
class Hydrology():
	...
	def contributingArea(self,dirGrid,asSurface = True):
		...
		# lower block
		for j in range(self.ang.nrows/2,self.ang.nrows-1):
			if self.progress: self.progress.setPercentage(100*float(j-self.ang.nrows/2)/self.ang.nrows) 
			for i in range(self.ang.ncols/2,self.ang.ncols-1): self.d8Area(j, i)
			for i in range(self.ang.ncols/2,0,-1): self.d8Area(j, i)
		# upper block
		for j in range(self.ang.nrows/2,0,-1):
